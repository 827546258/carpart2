package com.xiupeilian.carpart.sysconstant;
/*
* 这是一个常量类
*
* */
public class SysConstant {
    public static final String VALIDATE_CODE="validate";
    public static final Integer CITY_CHINA_ID=100000;
    public static final Integer ROLE_ADMIN=3;
    public static final Integer EmPLOYEE_RANK=1;
    //阿里云API的外网域名
    public static final String ENDPOINT = "oss-cn-beijing.aliyuncs.com";//"oss-cn-shanghai.aliyuncs.com";
    //阿里云API的密钥Access Key ID
    public static final String ACCESS_KEY_ID = "LTAICvhI2oyM59Qb";
    //阿里云API的密钥Access Key Secret
    public static final String ACCESS_KEY_SECRET = "T85kTo894T5Y21ePfWQxc0sWd7ztKw";
    //阿里云API的bucket名称
    public static final String BACKET_NAME = "xiaoyugg";//"uploadpicture";
    //阿里云API的文件夹名称
    public static final String FOLDER="imm/";

}
